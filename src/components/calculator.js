...
export default () => {
    [].forEach.call(
        document.querySelectorAll("form.has-calculator"),
        function (form) {
            form.addEventListener("change", function () {
                let product = form.querySelector('input[name="product"]');
                if (product.type !== "hidden") {
                    product = form.querySelector(
                        'input[name="product"]:checked'
                    );
                }
                let length = form.querySelector('input[name="length"]:checked');
                let interval = form.querySelector(
                    'input[name="interval"]:checked'
                );
                let policarbonat = form.querySelector(
                    'input[name="policarbonat"]:checked'
                );
                let sum = 0;
                try {
                    sum +=
                        calculator.products[product.value][length.value][
                            interval.value
                        ][policarbonat.value];

                    const animateBlock = form.querySelector(
                    ...
                    animateBlock.classList.add(
                        "animated",
                        "faster",
                        animationName
                    );
                    animateBlock.addEventListener("animationend", function () {
                        animateBlock.classList.remove(
                            "animated",
                            "faster",
                            animationName
                        );
                    ...
                    let numberFrom = Number(
                        typeof out.dataset.from === "undefined"
                            ? 0
                            : out.dataset.from
                    );
